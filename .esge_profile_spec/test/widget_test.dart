...
void main() {
  ...
  testWidgets('les réglages du profil ouvrent leurs pages', (tester) async {
    ...
    expect(find.text('Fonction'), findsOneWidget);
  ...
  testWidgets('afficher les éléments révèle les données de la fenêtre', (
    tester,
  ) async {
    await tester.pumpWidget(
      const MaterialApp(
    ...
    await tester.tap(find.text('Entrées'));
    await tester.pumpAndSettle();
    await tester.tap(find.text('Afficher les éléments'));
    await tester.pumpAndSettle();
    expect(find.text('Approvisionnement caisse'), findsOneWidget);
    expect(find.text('+ 5 000 000 F'), findsOneWidget);
    expect(tester.takeException(), isNull);
  });
}
