...
class _CrmPageState extends State<CrmPage> {
  static const color = Color(0xFF9B51E0);
  @override
  void initState() {
    super.initState();
  ...
  Widget build(BuildContext context) => BlocBuilder<CrmCubit, CrmState>(
    builder: (_, state) => FeatureScaffold(
      ...
      primaryAction: 'Ajouter un prospect',
      children: [
        const PipelineBar(),
        ...
        ...state.prospects.map(
          (item) => RecordCard(
            ...
            subtitle: '${item.contact} • ${item.owner} • ${item.need}',
            value: '${item.potential.toStringAsFixed(0)} F',
            color: color,
