...
class _RoleSelectionPageState extends State<RoleSelectionPage> {
  ...
  void initState() {
    super.initState();
    _roleTimer = Timer.periodic(const Duration(milliseconds: 2800), (_) {
      if (!mounted || !_roleController.hasClients) {
        return;
      }
      final current = UserRole.values.indexOf(selected);
      final next = (current + 1) % UserRole.values.length;
      _roleController.animateToPage(
        next,
                        ...
                        itemBuilder: (_, i) => AnimatedBuilder(
                          ...
                          builder: (_, child) {
                            ...
                            final signedDistance =
                                _roleController.hasClients &&
                                    _roleController.position.haveDimensions
                                ? (_roleController.page! - i).clamp(-1.0, 1.0)
                                : 0.0;
                            return Transform.rotate(
                              angle: signedDistance * .055,
                              child: Transform.scale(
                                scale: .72 + ((1 - distance) * .28),
                                child: Opacity(
                                  opacity: .48 + ((1 - distance) * .52),
                                  child: child,
                                ),
                              ),
                            );
